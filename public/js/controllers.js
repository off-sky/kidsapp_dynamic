...
.controller('menubarController', ['$scope', '$uibModal', 'userFactory', '$sce', function($scope, $uibModal, userFactory, $sce){
    ...
    $scope.testOperationPopoverContent = $sce.trustAsHtml('<div><h4 class="testop-popover-header">Сайт працює в тестовому режимі!</h4><p class="testop-popover-par">Радимо залогінитись, щоб оцінити всі функції цього сайту:<br>Ім\'я користувача: <strong>Admin</strong><br> Пароль: <strong>аdmin</strong><br></p></div>');
}])
...
.controller('loginModalController', ['$scope', '$uibModalInstance', 'userFactory', function($scope, $uibModalInstance, userFactory){
    ...
    $scope.doSignIn = function(){
        ...
        if (!($scope.newUser.username && $scope.newUser.username.length > 0) || !($scope.newUser.lastName && $scope.newUser.lastName.length > 0) || !($scope.newUser.firstName && $scope.newUser.firstName.length > 0) || !($scope.newUser.password && $scope.newUser.password.length > 0)) {
            return
        }
        $scope.signInError = false;
...
.controller('newsChangeModalController', ['$scope', '$uibModalInstance', 'newsToChange', function($scope, $uibModalInstance, newsToChange){
    ...
    $scope.saveChanges = function(){
        if (!($scope.newsToChange.title && $scope.newsToChange.title.length > 0) || !($scope.newsToChange.author && $scope.newsToChange.author.length > 0) || !($scope.newsToChange.text && $scope.newsToChange.text.length > 0) || !($scope.newsToChange.date && $scope.newsToChange.date.length > 0)) {
            return
        }
        var photoPreview = angular.element(document.querySelector('#img-preview'));
        var base64Image = photoPreview.attr('src');
